        ...
        if (client) {
            ...
            bot.sendMessage(userId, 
                ...
                { 
                    parse_mode: 'HTML',
                    reply_markup: {
                        inline_keyboard: [[{ text: '🔄 Повторить последний заказ', callback_data: `repeat_last` }]]
                    }
                }
            );
        } else {
            bot.sendMessage(userId, "Мы пока не знакомы! Сделайте первый заказ, чтобы появился профиль.");
        }
    } catch (e) {
        console.error(e);
        bot.sendMessage(userId, "Ошибка получения профиля.");
    }
});
...
bot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('rate|')) {
        ...
        bot.editMessageText(`✅ Спасибо! Вы поставили ${stars}⭐ заказу ${orderId}`, {
            chat_id: userId,
    ...
    try {
        ...
        if (action === 'status') {
            ...
            if (newStatus === 'Выполнен') {
                userNotifyText = `🎉 Заказ <b>${orderId}</b> доставлен! Приятного аппетита.`;
                askFeedback = true;
            }
...
async function sortSheetsByDate() {
     try {
        ...
        const otherSheets = []; const dateSheets = [];
        allSheets.forEach(sheet => {
            const title = sheet.properties.title;
            if (/^\d{2}\.\d{2}\.\d{4}$/.test(title)) dateSheets.push(sheet); else otherSheets.push(sheet);
        });
        dateSheets.sort((a, b) => {
            const dateA = parseDate(a.properties.title); const dateB = parseDate(b.properties.title); return dateA - dateB;
        });
        const sortedSheets = [...otherSheets, ...dateSheets];
        const requests = [];
        sortedSheets.forEach((sheet, index) => {
            if (sheet.properties.index !== index) requests.push({ updateSheetProperties: { properties: { sheetId: sheet.properties.sheetId, index: index }, fields: "index" } });
        });
...
async function updateDailySummary(sheetName) {
    try {
        ...
        const summaryData = [['📦 ИТОГО НА ДЕНЬ', 'КОЛ-ВО']]; for (const [name, qty] of Object.entries(totals)) summaryData.push([name, qty]);
        await sheets.spreadsheets.values.clear({ spreadsheetId: SPREADSHEET_ID, range: `${sheetName}!N1:O100` });
        await sheets.spreadsheets.values.update({ spreadsheetId: SPREADSHEET_ID, range: `${sheetName}!N1`, valueInputOption: 'USER_ENTERED', resource: { values: summaryData } });
    } catch (e) {}
}
...
function calculateOrderTotals(cart, products) {
    let totalItemsAmount = 0; let totalQty = 0;
    cart.forEach(item => { const product = products.find(p => p.id === item.id); if (product) { totalItemsAmount += product.price * item.qty; totalQty += item.qty; } });
    return { totalItemsAmount, finalTotal: totalItemsAmount };
}
...
app.get('/api/get_products', async (req, res) => {
    try {
        ...
        const products = rows.filter(row => row[7] === 'TRUE' || row[7] === 'Да' || row[7] === true).map((row, index) => ({ id: row[0], category: row[1], name: row[2], price: parseFloat(row[3]) || 0, description: row[4], imageUrl: row[5], stock: parseInt(row[6]) || 0, rowIndex: index + 2 }));
        const response = { status: 'success', products }; cache.set("products", response); res.json(response);
